...
class RankingFrame(tk.Frame):
    def __init__(self, parent, ranking_list, show_home):
        ...
        tk.Label(header, text="Food Ranking", font=("Segoe UI", 14, "bold")).pack(side="left", padx=12)
    ...
    def open_edit_popup(self):
        ...
        tk.Label(popup, text="New Rank (0-10):").grid(row=0, column=0, sticky="e", padx=6, pady=6)
        ...
        def submit():
            try:
                self.ranking_list.change_item_rank(name, float(rank_entry.get()))
                self.refresh_list()
                popup.destroy()
    ...
    def open_add_popup(self):
        ...
        tk.Label(popup, text="Rank (0-10):").grid(row=1, column=0, sticky="e", padx=6, pady=6)
        rank_entry = tk.Entry(popup)
        ...
        def submit():
            try:
                self.ranking_list.insert_new_item(name_entry.get(), float(rank_entry.get()))
                self.refresh_list()
                popup.destroy()
            except ValueError as e:
                error_label.config(text=str(e))
    ...
    def open_tier_list(self):
        ...
        for tier, items in self.ranking_list.get_tiers().items():
            row = tk.Frame(popup)
            row.pack(fill="x", anchor="w", padx=10, pady=2)
            tk.Label(row, text=f"{tier}", font=("Segoe UI", 11, "bold"), width=4, anchor="w").pack(side="left")
            tk.Label(row, text=", ".join(items) if items else "—", anchor="w").pack(side="left")
